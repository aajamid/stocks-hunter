...
test("14-day range uses ~7.143 points per day", () => {
  const [row] = scoreRows([createRow("A", 11, 3)], undefined, 14)
  assert.equal(row.score_components.pointPerDay, 7.143)
  assert.equal(row.score, 78.571)
})
...
test("28-day range uses ~3.571 points per day", () => {
  ...
  assert.equal(row.score, 82.143)
})

test("score is capped at zero when all days are down", () => {
  const [row] = scoreRows([createRow("D", 0, 14)], undefined, 14)
  assert.equal(row.score, 0)
})

test("down days deduct points from 100", () => {
...
test("flat movement yields full score", () => {
